...
func (sch *scheduler) selectPathRoundRobin(s *session, hasRetransmission bool, hasStreamRetransmission bool, fromPth *path) *path {
	//cyb
	//fmt.Println("---selectPathRoundRobin start ----")
	if sch.quotas == nil {
		sch.setup()
	}
...
pathLoop:
	for pathID, pth := range s.paths {
		...
		//cyb
		//fmt.Println("---selectPathRoundRobin->pathID",pathID,"----")
		...
		case *wire.StreamFrame:
			if frame.FinBit {
				...
				for pathID, pth := range s.paths {
					...
					fmt.Println("Path %x: sent %d retrans %d lost %d; rcv %d rtt %v", pathID, sntPkts, sntRetrans, sntLost, rcvPkts, pth.rttStats.SmoothedRTT())
					
					_, err = fmt.Fprintf(f, "%x,%x,%x,%d,%d,%d,%d,%v \n", frame.StreamID, s.connectionID, pathID, sntPkts, sntRetrans, sntLost, rcvPkts, pth.rttStats.SmoothedRTT())
					if err != nil {
						fmt.Println(err)
					}
				}
